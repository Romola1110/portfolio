 ...
 *
 * 【放图方式 · 不必编号】
 * 1. 把图片放进 assets/photos/spring/ 等文件夹，文件名随意（如 DSC08421.jpg、雨夜.jpg）
 * 2. 在下方 PHOTO_MANIFEST 对应季节登记文件名
 * 3. title / orient / diary / exif 可写可不写（不写则用文件名或默认占位）
 *
 * orient: 'landscape' 横图 | 'portrait' 竖图
 */

const PHOTO_SEASONS = {
  spring: { key: 'spring', label: '春 · 花信', en: 'Spring', hue: '142, 168, 136' },
  summer: { key: 'summer', label: '夏 · 长风', en: 'Summer', hue: '72, 140, 104' },
  autumn: { key: 'autumn', label: '秋 · 落木', en: 'Autumn', hue: '192, 107, 42' },
  winter: { key: 'winter', label: '冬 · 藏雪', en: 'Winter', hue: '138, 155, 184' }
};

/* ← 你的真实作品登记在这里（有内容时优先使用，不再要求 01.jpg 编号） */
const PHOTO_MANIFEST = {
  spring: [
    // { file: '雨夜.jpg', title: '雨夜', orient: 'landscape', diary: '那日有风…', exif: 'f/2.8 · 1/320s · ISO 200' },
  ],
  summer: [],
  autumn: [],
  winter: []
...
/* 以下为 Demo 占位数据：某季 manifest 为空时使用 */
const PHOTO_TITLES = {
...
const PHOTO_CAPTIONS = [
  '岁月未竟，光影尚存', '风物有信，四时皆书', '一瞬成诗，万象无声', '镜头深处，有风经过',
  '光阴在帧里轻轻落脚', '墨色渐散，景致自明', '山河入镜，心事微澜', '静看流转，如墨入水',
  '半卷光影，满纸清寒', '此间风物，皆可入画', '行到水穷，坐看云起', '人间烟火，亦是风景',
  '浅照浮生，深留一念', '光落檐角，梦醒时分', '拾一片影，藏一季声', '镜头不语，岁月作答',
  '远意近收，都在光里', '风起时，画面有了呼吸', '片刻凝驻，长久回响', '四时轮转，光影为证'
];
...
const PHOTO_DIARY = [
  ...
  '在旧巷口徘徊许久，只为等这一束斜照。', '镜头里留着未说完的话，像晾着的字笺。',
  '光落檐角，梦醒时分，恰好路过。', '行到水穷，云起时按下一张。',
  '薄暮将至，色彩忽然变得温柔。', '想留住风经过树叶的那一秒。',
  '远处有钟，近处是静默的影。', '此间风物，皆出自我手与眼。',
  '冬日午后，炉边光影极淡极长。', '春信来时，枝头先绿了一寸。',
  '夏夜潮声远，近处只剩路灯。', '秋深叶落，石径上响着细碎声。',
  '港城夜雨，霓虹在水面揉碎。', '游园偶得，不贵重，却想分享。',
  '纸窗透进半阙月色，便足矣。', '苔痕上阶绿，镜头比诗更慢。',
  '风起时，画面忽然有了呼吸。', '片刻凝驻，长久回响。'
];
...
function photoFallbackSrc(season, seed, orient) {
  if (orient === 'portrait') {
    return `https://picsum.photos/seed/${season}-${seed}/400/520`;
  }
...
function normalizeManifestEntry(entry, i, season) {
  ...
  return {
    ...
    exif: (typeof entry === 'object' && entry.exif)
      || `f/${(1.8 + (i % 4) * 0.7).toFixed(1)} · 1/${200 + i * 17}s · ISO ${100 + (i % 5) * 100}`,
    orient,
    aspect,
...
function buildDemoItems(season) {
  ...
  return titles.map((title, i) => {
    const id = i + 1;
    const file = `${String(id).padStart(2, '0')}.jpg`;
...
function buildPhotoItems(season) {
  ...
  if (manifest.length) {
    return manifest.map((entry, i) => normalizeManifestEntry(entry, i, season));
  }
